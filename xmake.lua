...
target("lab2")
    ...
    after_build(function (target)
        os.cp("src/VCX/Labs/2-FluidSimulation/shaders/*", path.join(target:targetdir(), "assets", "shaders"))
    end)
    after_install(function (target)
        os.cp("src/VCX/Labs/2-FluidSimulation/shaders/*", path.join(target:installdir(), "bin", "assets", "shaders"))
    end)
