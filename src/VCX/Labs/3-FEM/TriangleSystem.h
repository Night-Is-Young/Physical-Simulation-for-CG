...
#include "Labs/3-FEM/Triangle.h"
#include <glm/glm.hpp>
#include <vector>

namespace VCX::Labs::FEM {
    struct TriangleSystem {
        float Lx { 2.0f }, Ly { 2.0f }; // Lengths of the system in x, y, z directions
        int   nx { 32 }, ny { 32 };       // Number of subdivisions in x, y, z directions
        glm::vec4 facecolor { 0.0f, 0.5f, 0.3f, 0.5f };
        glm::vec4 edgecolor { 1.0f, 1.0f, 1.0f, 1.0f };

        float _youngs_modulus { 50.0f }; // Young's modulus for the material
